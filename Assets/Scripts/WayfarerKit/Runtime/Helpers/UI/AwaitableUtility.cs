...
namespace WayfarerKit.Runtime.Helpers.UI
{
	/// <summary>
...
#region WhenAll_Four_Results
		...
		private static Exception CreateAggregateException(Exception exception) => throw (exception is AggregateException ? exception : new AggregateException(exception));
		...
		/// </returns>
		private static Exception CreateAggregateException(params Exception[] exceptions)
        {
            if (!exceptions.Any(e => e is OperationCanceledException)) return new AggregateException(exceptions).Flatten();

            if (exceptions.All(e => e is OperationCanceledException)) return new AggregateException(exceptions[0]);

            return new AggregateException(exceptions.Where(e => e is not OperationCanceledException)).Flatten();
        }

        private static class Result<TResult>
        {
...
#region WhenAll_Any_Results
		...
		public static async Awaitable WhenAll(Awaitable[] awaitables)
        {
            ...
            if (exceptionCount > 0) throw CreateAggregateException(exceptions);
        }
